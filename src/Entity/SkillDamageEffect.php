...
use App\Repository\SkillDamageEffectRepository;
use Doctrine\Common\Collections\ArrayCollection;
use Doctrine\Common\Collections\Collection;
use Doctrine\ORM\Mapping as ORM;

/**
 * @ORM\Entity(repositoryClass=SkillDamageEffectRepository::class)
 */
class SkillDamageEffect extends Entity
{

    const IGNORE_DEFENSE_CHOICES = [
        "Effect reacts normally to defense" => false,
        "Effect ignore targets defenses" => true
    ];
    ...
    /**
     * @ORM\ManyToOne(targetEntity=Skill::class, inversedBy="skillDamageEffects")
    ...
    /**
     * @ORM\ManyToOne(targetEntity=Weapon::class, inversedBy="skillDamageEffects")
     * @ORM\JoinColumn(nullable=true, onDelete="CASCADE")
     */
    private ?Weapon $weapon;

    /**
     * @ORM\ManyToOne(targetEntity=Consumable::class, inversedBy="skillDamageEffects")
     * @ORM\JoinColumn(nullable=true, onDelete="CASCADE")
     */
    private ?Consumable $consumable;

    /**
     * @ORM\ManyToOne(targetEntity=Action::class, inversedBy="skillDamageEffects")
     * @ORM\JoinColumn(nullable=true, onDelete="CASCADE")
     */
    private ?Action $action;

    /**
     * @ORM\OneToMany(targetEntity=Target::class, mappedBy="skillDamageEffect")
     */
    private ?Collection $targets;

    /**
     * @ORM\ManyToMany(targetEntity=PotencyAugmentator::class, inversedBy="skillDamageEffects")
     */
    ...
    public function addTarget(Target $target): self
    {
        if (!$this->targets->contains($target)) {
            $this->targets[] = $target;
            $target->setSkillDamageEffect($this);
        }
    ...
    public function removeTarget(Target $target): self
    {
        if ($this->targets->contains($target)) {
            ...
            if ($target->getSkillDamageEffect() === $this) {
                $target->setSkillDamageEffect(null);
            }
        }
